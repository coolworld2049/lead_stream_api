...
def create_fastapi_app() -> FastAPI:
    ...
    @app.get(
        "/{secure_path}/docs", response_class=HTMLResponse, include_in_schema=False
    )
    async def get_docs(secure_path: str) -> HTMLResponse:
        check_secure_path(secure_path)
        return get_swagger_ui_html(
            openapi_url=f"/api/{secure_path}/openapi.json", title="docs"
        )

    @app.get(
        "{secure_path}/redoc", response_class=HTMLResponse, include_in_schema=False
    )
    async def get_redoc(secure_path: str) -> HTMLResponse:
        check_secure_path(secure_path)
        return get_redoc_html(
            openapi_url=f"/api/{secure_path}/openapi.json", title="red"
        )
